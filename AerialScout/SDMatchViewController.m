...
- (void) tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(selectedMatch.isCompleted == 31) {
        [[self navigationController] pushViewController:newView animated:YES];
    } else {
        [[self navigationController] pushViewController:newView animated:NO];
        SDMatch* selectedMatchCopy = [[SDMatch alloc] initWithCopy:selectedMatch];
        if(selectedMatchCopy.finalResult == 3) selectedMatchCopy.isCompleted = 31;
        ...
        int newIndex = !(selectedMatch.isCompleted & 2)  ? 1:
                       ...
                       !(selectedMatch.isCompleted & 16) ? 4:
                                                           0;
        
        [[SDViewServer getInstance] showNewViewFor:newView oldIndex:-1 newIndex:newIndex matchData:selectedMatch matchCopy:selectedMatchCopy];
    }
}
...
- (IBAction) addNewMatch:(id)sender {
    ...
    [[SDViewServer getInstance] showNewViewFor:newView oldIndex:-1 newIndex:0 matchData:newMatch matchCopy:newMatch];
}
